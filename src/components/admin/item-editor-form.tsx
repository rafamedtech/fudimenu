...
import type { Category, Locale, MenuItem, VisibilityScheduleFields } from '@/types/domain';
...
export function ItemEditorForm({ initial, categories, sectionId, offlineConflictId, defaultLocale }: Props) {
  ...
  const {
    ...
    defaultValues: {
      ...
      scheduleStartDate: initial?.scheduleStartDate ?? null,
      scheduleEndDate: initial?.scheduleEndDate ?? null,
  ...
  const scheduleValue: VisibilityScheduleFields = {
    scheduleDays: watch('scheduleDays') ?? [],
    scheduleStartMinute: watch('scheduleStartMinute') ?? null,
    scheduleEndMinute: watch('scheduleEndMinute') ?? null,
    scheduleStartDate: watch('scheduleStartDate') ?? null,
    scheduleEndDate: watch('scheduleEndDate') ?? null,
  };

  function patchSchedule(patch: Partial<VisibilityScheduleFields>) {
    for (const [key, val] of Object.entries(patch)) {
      setValue(key as keyof VisibilityScheduleFields, val as never, {
        shouldDirty: true,
        shouldValidate: true,
      });
    }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="relative flex flex-col gap-4">
        ...
        <Card className="border border-ink-200 bg-[var(--brand-surface)] shadow-sm">
          <ScheduleControls
            ...
            description="Opcional. Controla cuándo aparece este platillo en tu menú público. No afecta disponibilidad ni inventario. Sin selección = siempre visible."
            errors={{
              scheduleEndMinute: errors.scheduleEndMinute?.message,
              scheduleEndDate: errors.scheduleEndDate?.message,
            }}
          />
        </Card>
